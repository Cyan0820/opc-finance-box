...
class ShipBobShadowRequestError(ValueError):
    """Raised when a ShipBob monthly Shadow request is unsafe or out of scope."""
...
def _period_bounds(period: str) -> tuple[str, str]:
    if not isinstance(period, str) or not PERIOD_PATTERN.fullmatch(period):
        raise ShipBobShadowRequestError("period must use YYYY-MM")
...
def _ensure_entity_scope(runtime: BoxRuntime, entity_id: str) -> None:
    try:
        ...
        raise ShipBobShadowRequestError(str(exc)) from exc
...
def _write_private_json(output: str | Path, value: dict[str, Any]) -> Path:
    ...
    try:
        ...
        raise ShipBobShadowRequestError(
            "ShipBob Shadow request output already exists"
        ) from exc
    try:
...
def build_shipbob_shadow_request(
    runtime: BoxRuntime,
    *,
    entity_id: str,
    period: str,
    ...
    """Create one complete production ShipBob order/return request."""
...
def read_private_shipbob_shadow_request(path: str | Path) -> dict[str, Any]:
    source = Path(path).expanduser().resolve()
    if source.is_symlink() or not source.is_file():
        raise ShipBobShadowRequestError(
            "ShipBob Shadow request must be a regular non-symlink file"
        )
    metadata = source.stat()
    if os.name != "nt" and stat.S_IMODE(metadata.st_mode) != 0o600:
        raise ShipBobShadowRequestError("ShipBob Shadow request must use mode 0600")
    if not 0 < metadata.st_size <= MAX_REQUEST_BYTES:
        raise ShipBobShadowRequestError(
            "ShipBob Shadow request must be between 1 byte and 256 KiB"
        )
    try:
        request = json.loads(source.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise ShipBobShadowRequestError(
            "ShipBob Shadow request must contain valid UTF-8 JSON"
        ) from exc
    if not isinstance(request, dict):
        raise ShipBobShadowRequestError("ShipBob Shadow request must be a JSON object")
    return request


def validate_shipbob_shadow_request(
    runtime: BoxRuntime,
    request: dict[str, Any],
) -> dict[str, Any]:
    """Validate one complete ShipBob request without reading credentials or the network."""
    if not isinstance(request, dict) or set(request) != {"pipeline_id", "payload"}:
        raise ShipBobShadowRequestError(
            "ShipBob Shadow request fields must be pipeline_id and payload"
        )
    if request.get("pipeline_id") != PIPELINE_ID:
        raise ShipBobShadowRequestError(
            f"ShipBob Shadow request requires pipeline_id {PIPELINE_ID}"
        )
    payload = request.get("payload")
    if not isinstance(payload, dict) or set(payload) != {
        "entity_id", "period", "shipbob_request",
    }:
        raise ShipBobShadowRequestError("ShipBob Shadow request payload fields are invalid")
    if _contains_secret(request):
        raise ShipBobShadowRequestError(
            "credentials and ShipBob merchant bindings are prohibited in the request"
        )
    entity_id = payload.get("entity_id")
    if not isinstance(entity_id, str) or not entity_id:
        raise ShipBobShadowRequestError("payload.entity_id is required")
    _ensure_entity_scope(runtime, entity_id)
    period = payload.get("period")
    interval_start, interval_end = _period_bounds(period)
    connector_request = payload.get("shipbob_request")
    expected = {
        "mode": "fetch",
        "default_entity_id": entity_id,
        "environment": "production",
    ...
    if not isinstance(connector_request, dict) or set(connector_request) != set(expected):
        raise ShipBobShadowRequestError("ShipBob Shadow connector request fields are invalid")
    if connector_request != expected:
        raise ShipBobShadowRequestError(
            "ShipBob Shadow request must use production fetch, exact month bounds and bounded pagination"
        )
    try:
        ...
        raise ShipBobShadowRequestError(
            "ShipBob Shadow request must use real UTC month bounds"
        ) from exc
    if end <= start:
        raise ShipBobShadowRequestError(
            "ShipBob Shadow request must use an increasing UTC month window"
        )
    return {
        "valid": True,
        "pipeline_id": PIPELINE_ID,
...
def verify_private_shipbob_shadow_request(
    runtime: BoxRuntime,
    path: str | Path,
) -> dict[str, Any]:
    return validate_shipbob_shadow_request(
        runtime,
