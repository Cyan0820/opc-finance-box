...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the offline WooCommerce Connector Pack contract")
    ...
    request = json.loads((PACK_DIR / "fixture-order-refunds.json").read_text(encoding="utf-8"))
    report = run_connector_contract_test(
        registry, runtime, "woocommerce.order_refund_activity", request,
