...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the offline Wise Connector Pack contract")
    ...
    request = json.loads(
        (PACK_DIR / "fixture-balance-statement.json").read_text(encoding="utf-8")
    )
    report = run_connector_contract_test(
        registry, runtime, "wise.balance_statement", request,
